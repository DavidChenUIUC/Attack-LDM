...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                ...
                if cond_key in ['caption', 'coordinates_bbox', "txt"]:
                    ...
                    # print("734") ## Running here
                    attack=True
                    clean_xc = super().get_input(batch, cond_key).to(self.device)
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    # print("741") ## Running here
                    clean_c = self.get_learned_conditioning(clean_xc.to(self.device))
                    attacked_c = self.get_learned_conditioning(attacked_xc.to(self.device))
            else:
                c = xc
            if bs is not None:
                c = c[:bs]
        ...
        out = [z, clean_c, attacked_c] ## Both clean_c, and attacked_c are positional encodeded
