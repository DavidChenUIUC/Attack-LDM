...
class DDIMSampler(object):
    ...
    def sample(self,
        ...
        if attacked_conditioning is not None:
            if isinstance(attacked_conditioning, dict):
                ctmp = attacked_conditioning[list(attacked_conditioning.keys())[0]]
                while isinstance(ctmp, list): ctmp = ctmp[0]
                cbs = ctmp.shape[0]
                if cbs != batch_size:
                    print(f"Warning: Got {cbs} attacked_conditioning but batch-size is {batch_size}")

            else:
                if attacked_conditioning.shape[0] != batch_size:
                    print(f"Warning: Got {attacked_conditioning.shape[0]} attacked_conditionings but batch-size is {batch_size}")
        ...
        samples, intermediates = self.ddim_sampling(conditioning, attacked_conditioning, size,
                                                    callback=callback,
    ...
    @torch.no_grad()
    def ddim_sampling(self, cond, attacked_cond, shape,
